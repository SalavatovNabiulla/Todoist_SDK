...
Функция ПолучитьСписокВсехПроектов()  Экспорт
	...
	Если ОбработатьОтвет(Ответ) Тогда
		...
		Для Каждого Стр Из ДанныеJSON Цикл
			...
			Если Стр.view_style = "board" Тогда
				ДанныеСтрока.СпособОтображенияДоска = Истина;
			Иначе
				ДанныеСтрока.СпособОтображенияДоска = Ложь;
			КонецЕсли;
...
Функция СоздатьНовыйПроект(Данные = Неопределено) Экспорт
	...
	Если ОбработатьОтвет(Ответ) Тогда
		...
		Если ДанныеJSON.view_style = "board" Тогда
			ДанныеСтрока.СпособОтображенияДоска = Истина;
		Иначе
			ДанныеСтрока.СпособОтображенияДоска = Ложь;
		КонецЕсли;	
...
Функция ПолучитьПроект(Данные = Неопределено) Экспорт
	...
	КонецЕсли;
	//
	Если Данные.Идентификатор = "" Тогда
		Возврат Неопределено;
	ИначеЕсли Данные.Идентификатор = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/projects/"+Данные.Идентификатор;
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
КонецФункции
//
Функция УдалитьПроект(Данные = Неопределено) Экспорт
	Если Данные = Неопределено Тогда
		Данные = ПолучитьСтруктуруДанных();
	КонецЕсли;
	//
...
КонецФункции
//
Функция ПолучитьУчастниковПроекта(Данные = Неопределено) Экспорт
	Если Данные = Неопределено Тогда
		Данные = ПолучитьСтруктуруДанных();
	КонецЕсли;
	//
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/projects/"+Данные.Идентификатор+"/collaborators";
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция ПолучитьСекцию(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/sections/"+Данные.Идентификатор;
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция СоздатьНовуюЗадачу(Данные = Неопределено) Экспорт
	...
	Если Данные.ИдентификаторИсполнителя <> Неопределено  Тогда
		ДанныеТела.Вставить("assignee_id",Данные.ИдентификаторИсполнителя);	
	КонецЕсли;
	//
	HTTPЗапрос.УстановитьИмяФайлаТела(ПолучитьJSONФайл(ДанныеТела));
	Ответ = HTTPСоединение.ВызватьHTTPМетод("POST",HTTPЗапрос);
...
Функция ПолучитьЗадачу(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/tasks/"+Данные.Идентификатор;
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция ОбновитьЗадачу(Данные = Неопределено) Экспорт
	...
	КонецЕсли;
	//
	Если Данные.СрокИсполнения <> Неопределено  Тогда
...
Функция ЗакрытьЗадачу(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/tasks/"+Данные.Идентификатор+"/close";
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция АктивироватьЗакрытуюЗадачу(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/tasks/"+Данные.Идентификатор+"/reopen";
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
КонецФункции
//
Функция УдалитьЗадачу(Данные = Неопределено) Экспорт
	Если Данные = Неопределено Тогда
		Данные = ПолучитьСтруктуруДанных(2);
	КонецЕсли;
	//
...
Функция СоздатьНовыйКомментарий(Данные = Неопределено) Экспорт
	...
	КонецЕсли;
	//
	Если Данные.ИдентификаторЗадачи = "" Или Данные.ИдентификаторПроекта = "" Тогда
		Возврат Неопределено;
	ИначеЕсли Данные.ИдентификаторЗадачи = Неопределено И Данные.ИдентификаторПроекта = Неопределено Тогда
		Возврат Неопределено;
...
Функция ПолучитьКомментарий(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/comments/"+Данные.Идентификатор;
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция ПолучитьЛичнуюМетку(Данные = Неопределено) Экспорт
	...
	//
	HTTPСоединение = ПолучитьСоединение();
	Адрес = "/rest/v2/labels/"+Данные.Идентификатор;
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры(Адрес,Параметры),ПолучитьЗаголовки());
...
Функция ПолучитьСтруктуруДанных(НомерСтруктуры = 0) Экспорт
	...
	Если НомерСтруктуры = 0 Тогда // Проект
		...
		Шаблон.Вставить("СпособОтображенияДоска",Ложь);
